  ...
  let f = function(context, callback) {
    ...
    let endCallback = function (err, context) {
      ...
      if (isAcknowledgeRequired(requestSpec)) {
        ...
        //18-11-2019: Changed to enable the input as array
        const outgoingDataSplit = outgoing.data.split(',');
        socketio.emit(outgoing.channel, ...outgoingDataSplit , function () {
          let response = {
        ...
        //18-11-2019: Changed to enable the input as array
		const outgoingDataSplit = outgoing.data.split(',');
		socketio.emit(outgoing.channel, ...outgoingDataSplit);
        markEndTime(ee, context, startedAt);
        return callback(null, context);
      }
    };
